...
// Header with title + property strip, then tabs (Resumo / Validações / Atividade).

const ALL_KEY = '__resumo__';

// ----- Property pill in the strip -----
...
function MiniStatus({ status, size = 'sm' }) {
  ...
  return (
    <span className={`inline-flex shrink-0 items-center gap-1 rounded-md font-semibold uppercase tracking-wider ${padding} ${
      v ? 'bg-[#E0F2E7] text-[#1A7A4A]' : 'bg-[#FCE6E3] text-[#9A2A1F]'
    }`}>
...
// ----- Sidebar items (light, with section grouping by status — ClickUp-like) -----
function SidebarItem({ active, status, n, titulo, hasComment, onClick }) {
  const dot = status === 'valido' ? '#1A7A4A' : '#C0392B';
  return (
    <button
      onClick={onClick}
      className={`relative flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left transition ${
        active
          ? 'bg-brand-50 text-brand-900 shadow-[inset_0_0_0_1px_#A0D2FC]'
          : 'text-ink-700 hover:bg-white'
      }`}
    >
      <span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: dot }}></span>
      <span className="w-5 shrink-0 font-mono text-[10.5px] text-ink-500 tabular-nums">
        {String(n).padStart(2, '0')}
      </span>
      <span className={`flex-1 truncate text-[12px] leading-tight ${active ? 'font-semibold' : 'font-medium'}`}>
        {titulo}
      </span>
      {hasComment && (
        <Icon.MessageSquare size={10} color={active ? '#005CA9' : '#9EB2BB'} />
      )}
    </button>
  );
}
      ...
      <div className="overflow-hidden rounded-md border border-ink-100 bg-white">
        <div className="flex items-center gap-2 border-b border-ink-100 px-3 py-1.5" style={{ backgroundColor: c.bg + '60' }}>
          ...
          <span className="text-[10.5px] text-ink-500">
            {savedComment.autor} · {savedComment.timestamp}
          </span>
          <button
            onClick={() => { setTipo(savedComment.tipo); setTexto(savedComment.texto); setEditing(true); }}
        ...
        <p className="px-3 py-2 text-[12.5px] leading-relaxed text-ink-900">
          {savedComment.texto}
        </p>
      </div>
    );
        ...
        <div className="flex items-center gap-1">
          {editing && (
            <button
              ...
              className="rounded-md px-2 py-1 text-[11px] font-medium text-ink-500 hover:bg-white"
            >
              Cancelar
            </button>
          )}
          <button
            onClick={() => {
              if (!canSave) return;
              onSave({
                tipo, texto: texto.trim(), autor: 'KS',
  ...
  return (
    <div>
      <div className="flex items-center gap-2.5 pb-3 border-b border-ink-100">
        <span className="grid h-7 w-7 place-items-center rounded-md font-mono text-[10.5px] font-bold tabular-nums text-white"
              style={{ backgroundColor: isInvalid ? '#C0392B' : '#005CA9' }}>
          {String(v.n).padStart(2, '0')}
        </span>
        <h3 className="flex-1 text-[15.5px] font-semibold leading-tight tracking-tight text-ink-900 truncate">
          {v.titulo}
        </h3>
        ...
        <div className="mt-5">
          <div className="mb-2 flex items-baseline justify-between text-[10.5px]">
            ...
            <span className="text-ink-500 tabular-nums">
              {v.subitems.filter((s) => s.status === 'valido').length}/{v.subitems.length} válidas
            </span>
          </div>
...
function ResumoRow({ label, value, mono, accent }) {
  return (
    <div className="flex items-center justify-between gap-3 py-1.5">
      ...
      <dd className={`text-[12px] ${accent ? 'font-semibold text-brand-800' : 'font-medium text-ink-900'} ${mono ? 'font-mono tabular-nums' : ''}`}>
        {value}
      </dd>
    </div>
  );
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {c.tags.map((t) => {
            const IconComp = Icon[t.icon] || Icon.Info;
            return (
              <div key={t.label} className="inline-flex items-center gap-1.5 rounded-md border border-ink-100 bg-white px-2.5 py-1 text-[11.5px]">
                <IconComp size={11} color="#005CA9" />
            ...
            {commented.map((key) => {
              ...
              return (
                <div key={key} className="flex items-start gap-2 rounded-md border border-ink-100 bg-white px-3 py-2">
                  <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full text-[9px] font-bold text-white"
                        style={{ background: 'linear-gradient(135deg,#00437A,#2D8AD8)' }}>
                    {cmt.autor}
                  </span>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      <span className="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[9.5px] font-semibold text-white"
...
function DetailDialog({ id, onClose, allRows, onNavigate }) {
  ...
  useEffect(() => {
    setSelected(firstInvalid ? firstInvalid.n : ALL_KEY);
  ...
  const overallStatus =
    ...
    validCount === total ? 'aprovado' : 'pendente';
  ...
  const selectedV = typeof selected === 'number'
    ? MOCK.VALIDATIONS.find((v) => v.n === selected)
    : null;
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      data-screen-label="DSB-001 Dialog"
    >
      <style>{`
      ...
      <div
        className="relative flex h-full max-h-[900px] w-full max-w-[1240px] flex-col overflow-hidden rounded-xl bg-white shadow-pop"
        style={{ animation: 'popIn 240ms cubic-bezier(.22,.61,.36,1) both' }}
      >
        {/* Top bar */}
          ...
          <div className="ml-auto flex items-center gap-1">
            <div className="flex items-center gap-0.5 rounded-md border border-ink-100 p-0.5">
              <button
                ...
                disabled={!prevId}
                className="grid h-6 w-6 place-items-center rounded hover:bg-ink-50 disabled:opacity-30 disabled:hover:bg-transparent"
              ...
              >
                <Icon.ChevronRight size={12} color="#404B52" />
              </button>
            </div>
            <button className="grid h-7 w-7 place-items-center rounded-md hover:bg-ink-50" title="Mais opções">
              <Icon.MoreHorizontal size={13} color="#404B52" />
            </button>
            <button
              onClick={onClose}
              className="grid h-7 w-7 place-items-center rounded-md border border-ink-100 hover:bg-ink-50"
              title="Fechar (Esc)"
            >
              <Icon.X size={13} color="#404B52" />
            </button>
          </div>
        </div>
        ...
        <div className="border-b border-ink-100 px-6 pt-4 pb-3">
          <div className="flex items-center gap-2">
            <button
              className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-[11px] font-bold uppercase tracking-wider shadow-sm"
              style={{ backgroundColor: ovMeta.bg, color: ovMeta.fg }}
            >
              <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: ovMeta.dot }}></span>
              {ovMeta.label}
              <Icon.ChevronDown size={11} color={ovMeta.fg} />
            </button>
            <span className="text-[11.5px] text-ink-500">
              · {validCount}/{total} validações OK
            </span>
            <div className="ml-auto flex items-center gap-1">
              <button className="inline-flex items-center gap-1 rounded-md border border-ink-100 bg-white px-2 py-1 text-[11px] font-medium text-ink-700 hover:bg-ink-50">
                <Icon.Paperclip size={11} color="#64747A" />
                Anexar
              </button>
              <button className="inline-flex items-center gap-1 rounded-md border border-ink-100 bg-white px-2 py-1 text-[11px] font-medium text-ink-700 hover:bg-ink-50">
                <Icon.Tag size={11} color="#64747A" />
                Tags
              </button>
            </div>
          </div>

          <h2 className="mt-2.5 text-[22px] font-semibold leading-tight tracking-tight text-ink-900">
            {c.beneficiario.nome}
          </h2>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-1.5">
            ...
            <PropPill label="% Obra" value={`${c.financiamento.pctObra}%`} icon="Percent" accent="#005CA9" />
            <span className="mx-0.5 h-4 w-px bg-ink-100"></span>
            <div className="inline-flex items-center gap-1.5 rounded-md border border-ink-100 bg-white px-2 py-1 text-[11.5px]">
              <Icon.User size={11} color="#64747A" />
              ...
              <span className="grid h-4 w-4 place-items-center rounded-full text-[8.5px] font-bold text-white"
                    style={{ background: 'linear-gradient(135deg,#00437A,#2D8AD8)' }}>KS</span>
              <span className="font-semibold text-ink-900">Karina Souto</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 border-b border-ink-100 bg-[#FAFBFD] px-6 py-2">
          <div className="text-[11px] font-bold tabular-nums uppercase tracking-wider text-ink-500">
            Progresso
          </div>
          <div className="text-[12px] font-semibold tabular-nums text-ink-700">
            {validCount}/{total} válidas
          </div>
          ...
          {invalids.length > 0 && (
            <button
              onClick={goNextInvalid}
              className="inline-flex items-center gap-1.5 rounded-md border border-[#C0392B]/30 bg-white px-2.5 py-1 text-[11px] font-semibold text-[#9A2A1F] hover:bg-[#FCE6E3]"
            >
              <span className="h-1.5 w-1.5 rounded-full bg-err"></span>
              {invalids.length} inválidas
              <Icon.ArrowRight size={10} color="#9A2A1F" />
            </button>
          )}
        </div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[280px_1fr]">
          <aside className="overflow-y-auto border-r border-ink-100 bg-[#F8FAFC]">
            <div className="space-y-0.5 p-2">
              ...
              />
              <div className="px-2 pb-1 pt-2.5 text-[9.5px] font-bold uppercase tracking-wider text-ink-500">
                Validações da Macro
              </div>
              <SidebarGroup
                statusKey="invalido"
        ...
        <div className="flex items-center justify-between gap-3 border-t border-ink-100 bg-white px-6 py-2.5">
          <div className="flex items-center gap-1.5 text-[10.5px] text-ink-500">
            <kbd className="rounded border border-ink-100 bg-ink-50 px-1.5 py-0.5 font-mono">Esc</kbd>
            <span>fechar</span>
          ...
          <div className="flex items-center gap-2">
            <button className="rounded-md border border-ink-100 bg-white px-3 py-1.5 text-[11.5px] font-medium text-ink-700 hover:bg-ink-50">
              Salvar rascunho
            </button>
            <button className="rounded-md border border-err/30 bg-white px-3 py-1.5 text-[11.5px] font-semibold text-err hover:bg-[#FCE6E3]">
              Recusar
            </button>
            <button className="rounded-md bg-brand-700 px-3.5 py-1.5 text-[11.5px] font-semibold text-white shadow-sm hover:bg-brand-800">
              Aprovar desembolso
            </button>
          </div>
        </div>
