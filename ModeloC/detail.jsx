          ...
          <div className="border-t border-ink-100 px-5 pb-5 pt-4">
            ...
            {hasSubs ? (
              <div className="mt-4 space-y-2.5">
                {v.subitems.map((sub) => (
                  <SubValidation key={sub.n} sub={sub} parentN={v.n} commentKey={`v${v.n}-${sub.n}`} comments={comments} onSaveComment={onSaveComment} />
                ))}
              </div>
            ) : (
              <CommentBlock commentKey={commentKey} savedComment={comments[commentKey]} onSave={(c) => onSaveComment(commentKey, c)} />
...
function DetailView({ onBack }) {
  ...
  const saveComment = (key, comment) => setComments((prev) => ({ ...prev, [key]: comment }));
  ...
  const overallStatus = MOCK.VALIDATIONS.some((v) => v.status === 'invalido') ? 'pendencia' : validCount === totalCount ? 'aprovado' : 'pendente';

  return (
    <div className="slide-in mx-auto max-w-[1400px] px-6 py-8" data-screen-label="Detail DSB-001">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-[13px] font-medium text-ink-700 hover:bg-ink-50">
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-[24px] font-semibold leading-tight text-ink-900">Contrato {c.contrato} — {c.beneficiario.nome}</h1>
          <p className="mt-1 text-[13px] text-ink-500">{c.financiamento.programa} · {c.financiamento.fase} · {c.financiamento.numeroDesembolso} desembolso</p>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        {c.tags.map((t) => (<UI.TagChip key={t.label} {...t} />))}
      </div>
      <section className="mt-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h2 className="text-[18px] font-semibold text-ink-900">Validações da Macro</h2>
            <p className="mt-0.5 text-[12.5px] text-ink-500">Cada validação foi executada automaticamente pela macro. Expanda para adicionar tratativa.</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <div className="text-[11.5px] uppercase tracking-wide text-ink-500">Status geral</div>
              <div className="text-[15px] font-semibold tabular-nums text-ink-900">{validCount}/{totalCount} <span className="text-[12.5px] font-medium text-ink-500">válidas</span></div>
            </div>
            <div className="h-2 w-48 overflow-hidden rounded-full bg-ink-100">
              <div className="h-full rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: 'linear-gradient(90deg,#005CA9,#2D8AD8)' }}></div>
            </div>
          </div>
        </div>
        <div className="mt-4 space-y-2.5">
          {MOCK.VALIDATIONS.map((v) => (<ValidationItem key={v.n} v={v} comments={comments} onSaveComment={saveComment} />))}
        </div>
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-ink-100 bg-white p-4 shadow-card">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-lg bg-brand-50"><Icon.FileCheck2 size={18} color="#005CA9" /></div>
            <div>
              <div className="text-[13.5px] font-semibold text-ink-900">Finalizar análise</div>
              <div className="text-[12.5px] text-ink-500">Revise as validações inválidas e adicione tratativas antes de aprovar.</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button className="rounded-lg border border-ink-100 bg-white px-3 py-2 text-[13px] font-medium text-ink-700 hover:bg-ink-50">Salvar rascunho</button>
            <button className="rounded-lg border border-err/30 bg-white px-3 py-2 text-[13px] font-medium text-err hover:bg-[#FCE6E3]">Recusar desembolso</button>
            <button className="rounded-lg bg-brand-700 px-4 py-2 text-[13px] font-semibold text-white shadow-sm hover:bg-brand-800">Aprovar desembolso</button>
          </div>
        </div>
      </section>
    </div>
