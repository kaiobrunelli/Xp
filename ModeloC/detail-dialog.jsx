...
function ResumoPanel({ comments }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <div className="mb-2 text-[10.5px] font-bold uppercase tracking-wider text-ink-500">Atributos do contrato</div>
...
function DetailDialog({ id, onClose, allRows, onNavigate }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-4" style={{ animation: 'fadeIn 200ms ease-out' }} role="dialog" aria-modal="true" data-screen-label="DSB-001 Dialog">
      ...
      <div className="relative flex h-full max-h-[900px] w-full max-w-[1240px] flex-col overflow-hidden rounded-xl bg-white shadow-pop" style={{ animation: 'popIn 240ms cubic-bezier(.22,.61,.36,1) both' }}>
        <div className="flex items-center gap-2 border-b border-ink-100 px-5 py-2">
          ...
          <div className="ml-auto flex items-center gap-1">
            <div className="flex items-center gap-0.5 rounded-md border border-ink-100 p-0.5">
              <button onClick={() => prevId && onNavigate(prevId)} disabled={!prevId} className="grid h-6 w-6 place-items-center rounded hover:bg-ink-50 disabled:opacity-30 disabled:hover:bg-transparent" title="Contrato anterior (←)"><Icon.ChevronLeft size={12} color="#404B52" /></button>
              <span className="px-1.5 text-[10.5px] font-semibold tabular-nums text-ink-700">{idx + 1}/{allRows.length}</span>
              <button onClick={() => nextId && onNavigate(nextId)} disabled={!nextId} className="grid h-6 w-6 place-items-center rounded hover:bg-ink-50 disabled:opacity-30 disabled:hover:bg-transparent" title="Próximo contrato (→)"><Icon.ChevronRight size={12} color="#404B52" /></button>
            </div>
            <button className="grid h-7 w-7 place-items-center rounded-md hover:bg-ink-50" title="Mais opções"><Icon.MoreHorizontal size={13} color="#404B52" /></button>
            <button onClick={onClose} className="grid h-7 w-7 place-items-center rounded-md border border-ink-100 hover:bg-ink-50" title="Fechar (Esc)"><Icon.X size={13} color="#404B52" /></button>
          </div>
        </div>
        ...
        <div className="border-b border-ink-100 px-6 pt-4 pb-3">
          <div className="flex items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-1">
              <button className="inline-flex items-center gap-1 rounded-md border border-ink-100 bg-white px-2 py-1 text-[11px] font-medium text-ink-700 hover:bg-ink-50"><Icon.Paperclip size={11} color="#64747A" />Anexar</button>
              <button className="inline-flex items-center gap-1 rounded-md border border-ink-100 bg-white px-2 py-1 text-[11px] font-medium text-ink-700 hover:bg-ink-50"><Icon.Tag size={11} color="#64747A" />Tags</button>
            </div>
          </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-1.5">
            ...
            <div className="inline-flex items-center gap-1.5 rounded-md border border-ink-100 bg-white px-2 py-1 text-[11.5px]">
              ...
              <span className="grid h-4 w-4 place-items-center rounded-full text-[8.5px] font-bold text-white" style={{ background: 'linear-gradient(135deg,#00437A,#2D8AD8)' }}>KS</span>
          ...
          <section className="overflow-y-auto bg-white">
            <div className="px-6 py-5">
              {selected === ALL_KEY ? (
                ...
                <ValidationDetailPanel v={selectedV} comments={comments} onSaveComment={saveComment} />
              ) : null}
            </div>
          </section>
        </div>
